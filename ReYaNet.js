...
const ruleOptions = {
  apple: true, // 苹果服务
  microsoft: false, // 微软服务，关闭，对Github有bug
...
const regionOptions = {
  ...
  regions: [
    {
      name: "🇭🇰 香港",
      regex: /港|HK|🇭🇰|Hong Kong|HongKong|hongkong/i,
      ratioLimit: 2,
      icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure/IconSet/Color/Hong_Kong.png",
    },
    {
      name: "🇺🇲 美国",
      regex: /美|🇺🇲|波特兰|达拉斯|俄勒冈|凤凰城|费利蒙|硅谷|拉斯维加斯|洛杉矶|圣何塞|圣克拉拉|西雅图|芝加哥|US|United States/i,
      ratioLimit: 2,
      icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure/IconSet/Color/United_States.png",
    },
    {
      name: "🇯🇵 日本",
      regex: /日本|🇯🇵|川日|东京|大阪|泉日|埼玉|沪日|深日|JP|Japan/i,
      ratioLimit: 2,
...
function main(config) {
  ...
  config["proxy-groups"] = [
    {
      ...
      name: "默认节点",
      type: "select",
      proxies: ["自动选择","全部节点", ...proxyGroupsRegionNames, "直连"],
  ...
  config["proxy-groups"].push({
    name: "自动选择",  // 组名
    type: "url-test",     // 代理类型为 url-test
    url: "http://cp.cloudflare.com/generate_204",  // 用于测试延迟的URL
    interval: 300,        // 测试间隔，单位为秒
    tolerance: 50,        // 容忍的延迟波动范围
    proxies: config.proxies.map(proxy => proxy.name),  // 包含所有代理节点
    lazy: true,           // 开启延迟加载测试
    icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure/IconSet/Color/Auto.png"  // 图标（可选）
  });
  
  config["proxy-groups"].push({
    name: "全部节点",  // 策略组名称
    type: "select",       // 代理类型为 select
    proxies: config.proxies.map(proxy => proxy.name),  // 包含所有代理节点
    icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure/IconSet/Color/Available_1.png"  // 可选的图标
  });
  ...
  if (ruleOptions.bilibili) {
    ...
    config["proxy-groups"].push({
      ...
      name: "哔哩哔哩",
      type: "select",
      proxies: ["直连","默认节点","全部节点", ...proxyGroupsRegionNames],
      url: "https://www.bilibili.com/",
      icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure/IconSet/Color/bilibili_2.png",
    });
  }
  ...
  if (ruleOptions.biliintl) {
    ...
    config["proxy-groups"].push({
      ...
      name: "哔哩哔哩东南亚",
      type: "select",
      proxies: ["默认节点","全部节点", "直连", ...proxyGroupsRegionNames],
  ...
  config["proxy-groups"].push(
    ...
    },
    {
      ...
      name: "其他外网",
      type: "select",
      proxies: ["默认节点", "国内网站","全部节点", ...proxyGroupsRegionNames],
